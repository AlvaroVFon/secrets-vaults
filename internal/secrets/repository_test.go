...
func TestSecretsRepository_Create_DuplicatedKey(t *testing.T) {
	...
	first, err := NewSecret("db.password", "s3cret", consumerID, true, false)
	if err != nil {
		t.Fatalf("new secret: %v", err)
	}
	...
	dup, err := NewSecret("db.password", "other", otherConsumerID, true, false)
	if err != nil {
		t.Fatalf("new secret: %v", err)
	}
...
func TestSecretsRepository_Update_DuplicatedKey(t *testing.T) {
	...
	second, err := NewSecret("db.host", "localhost", consumerID, true, false)
	if err != nil {
		t.Fatalf("new secret: %v", err)
	}
